...
class FingerStateMaskGenerator(torch.nn.Module):
    ...
    def _process_finger(
        ...
        # Without a press in this window there is no locally observable active
        # state.  A press without a following release, however, remains active
        # through the end of the window and must contribute release negatives.
...
class DiscreteGesturesModule(BaseLightningModule):
    ...
    positive_class_weight : float
        Multiplicative BCE weight for positive gesture targets. A value of 1.0
        preserves the upstream unweighted loss.
    ...
    def __init__(
        ...
        # Apply the positive weight outside BCEWithLogitsLoss so it does not
        # become a persistent loss-module buffer. This keeps checkpoints made
        # before this option was introduced strictly loadable.
    ...
    def weighted_bce_loss(
        ...
        positive_scale = 1.0 + targets * (self.positive_class_weight - 1.0)
        return loss * positive_scale
    ...
    def _step(self, batch: Mapping[str, torch.Tensor], stage: str = "train") -> float:
        ...
        # Compute loss
        loss = self.weighted_bce_loss(preds, targets)
