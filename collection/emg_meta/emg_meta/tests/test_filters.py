...
def test_dc_offset_is_removed() -> None:
    bank = EmgDisplayFilterBank(channels=1, sample_rate=SAMPLE_RATE)
    output = [bank.process((250.0,))[0] for _ in range(1000)]
...
def test_50_hz_is_not_unconditionally_notched() -> None:
    assert response_rms(50.0) > 0.75


def test_midband_is_preserved() -> None:
    assert response_rms(40.0) > 0.85


def test_upper_band_rolls_off_before_nyquist() -> None:
    response_80 = response_rms(80.0)
    response_115 = response_rms(115.0)
    assert response_80 > 0.65
    assert response_115 < response_80 * 0.35
...
def test_alignment_review_hides_mains_sine_and_keeps_emg_burst() -> None:
    ...
    time = np.arange(int(analysis_rate * 1.3)) / analysis_rate
    ...
    channels, combined = review_activity_envelopes(raw, analysis_rate)
