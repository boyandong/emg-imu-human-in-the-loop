...
def train(data_root: Path, output_root: Path, epochs: int, batch_size: int,
          learning_rate: float, device_name: str) -> Path:
    torch.manual_seed(7); np.random.seed(7)
    device = torch.device(device_name if device_name != "auto" else ("cuda" if torch.cuda.is_available() else "cpu"))
    feature_cfg = MPFConfig()
    ...
    display_names = {"index_press": "食指按下", "index_release": "食指释放",
                     ...
                     "thumb_click": "拇指轻点", "thumb_down": "拇指向下",
                     "thumb_in": "拇指向内", "thumb_out": "拇指向外", "thumb_up": "拇指向上"}
    ...
    # Six sequential scales with two kernel-3 TDS blocks have a 253-frame
    # causal receptive field.  Together with the MPF left context and 40-sample
    # stride this needs 10,334 raw samples (5.167 s).  Use a 6 s live model
    # window so online inference has the same temporal context as training.
...
def main() -> None:
    ...
    args = parser.parse_args(); train(args.data_root, args.output_root, args.epochs, args.batch_size, args.learning_rate, args.device)
