...
from .features import MultiBandMatrixPowerFeatures
from .model import MPFTDSNetwork
...
def main() -> None:
    ...
    with h5py.File(args.data_root / row["dataset"], "r") as handle:
        signal = torch.from_numpy(handle["data"]["emg"][:16214].T).float()[None]
    features = MultiBandMatrixPowerFeatures()(signal)
    model = MPFTDSNetwork()
