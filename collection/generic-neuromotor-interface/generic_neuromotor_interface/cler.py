...
def debounce_events(
    ...
    """
    Apply Meta's debouncing rule: remove any second event inside the debounce
    interval, except a release preceded by a different gesture.
    ...
    for name, time in events[1:]:
        ...
        # Skip every nearby event except a non-repeated release.  In
        # particular, a non-release following a release is still debounced.
        if time_diff < debounce:
            current_is_release = name in release_events
            if not (current_is_release and name != prev_name):
                continue
