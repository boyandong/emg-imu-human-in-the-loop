...
def run(archive:Path,output:Path,phase:str,load_family:str|None,position_family:str|None)->None:
    ...
    families={};models={};predictions={};split_rows=[];diagnostics=[]
    for si,subject in enumerate(subjects,1):
        ...
        for scenario,train_mask,target_mask in scenarios:
            ...
            split_rows.append({'subject':subject,'scenario':scenario,'train':sorted(set(train.trials.tolist())),
                              'validation' if phase=='validation' else 'test':sorted(set(target.trials.tolist()))})
            ...
            for fi,(name,factory) in enumerate(active.items(),1):
                ...
                families[(subject,scenario,name)]=family
                scaler=StandardScaler().fit(train_features[name],sample_weight=train.sample_weight)
                source_z=scaler.transform(train_features[name]);target_z=scaler.transform(target_features[name])
                source_centers=np.stack([source_z[train.labels==h].mean(0) for h in range(4)])
                condition_values=target.loads if scenario=='load_shift' else target.positions
                for value in np.unique(condition_values):
                    centers=np.stack([target_z[(condition_values==value)&(target.labels==h)].mean(0) for h in range(4)])
                    dn=float(np.linalg.norm(centers-source_centers,axis=1).mean())
                    dg=float(np.mean([np.linalg.norm(centers[i]-centers[j]) for i in range(4) for j in range(i+1,4)]))
                    diagnostics.append({'dataset':'emg_fmg','phase':phase,'subject':subject,'scenario':scenario,'condition':value,
                        'family':name,'D_nuisance':dn,'D_gesture':dg,'J':dg/(dn+1e-12),'feature_dimension':dimensions[name],
                        'definition':'source-standardized EMG-window class centroids; external load is not voluntary force'})
            ...
            for mi,(model_name,members) in enumerate(specs.items(),1):
                ...
                models[(subject,scenario,model_name)]=(scaler,model,members)
                predictions[f's{subject}_{scenario}_{model_name}']=p
                predictions[f's{subject}_{scenario}_trials']=np.unique(target.trials)
                predictions[f's{subject}_{scenario}_labels']=y
    ...
    with (output/'fitted_states.pkl').open('wb') as handle:pickle.dump((families,models),handle)
    np.savez_compressed(output/'heldout_predictions.npz',**predictions)
    (output/'scenario_split_trial_ids.json').write_text(json.dumps(split_rows,indent=2),encoding='utf-8')
    with (output/'load_position_family_diagnostics.csv').open('w',newline='',encoding='utf-8') as handle:
