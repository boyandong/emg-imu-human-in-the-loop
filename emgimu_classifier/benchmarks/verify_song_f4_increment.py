"""Recompute Song F4 held-out trial scores from exported probabilities."""
...
def verify(source: Path, directory: Path, baseline_path: Path, runner: Path) -> dict:
    result_path = directory / "F4_INCREMENT_RESULTS.json"
    result = json.loads(result_path.read_text(encoding="utf-8"))
    baseline = json.loads(baseline_path.read_text(encoding="utf-8"))
    if (result["status"] != "exploratory_song_one_person_one_day_f4_conditional_increment"
        ...
        raise ValueError("Song F4 study provenance changed")
    ...
    if classes != ["fist", "index_pinch", "neutral", "open_hand"] or \
            set(result["arms"]) != {"F0", "F0_plus_SPD", "F0_plus_F4",
                                    "F0_plus_SPD_plus_F4"}:
    ...
    for name, digest in (("conditional_incremental_csv", "conditional_incremental_sha256"),
        ...
        if result[digest] != sha256(directory / result[name]):
            raise ValueError(f"Song F4 delivery table changed: {name}")
    ...
    if len(grouped) != 8 or len(rows) != 1136:
        raise ValueError("Song F4 prediction coverage changed")
    ...
    for table in (conditional_rows, paired_rows):
        if len(table) != 4 or {(row["split"], row["base_arm"], row["candidate_arm"])
            ...
            raise ValueError("Song F4 delivery table coverage changed")
    for row in conditional_rows:
        ...
        if int(row["trials"]) != result["scores"][split][candidate]["trials"] or any(
            ...
            raise ValueError("Song F4 conditional CSV does not replay")
    for row in paired_rows:
        split, base, candidate = row["split"], row["base_arm"], row["candidate_arm"]
        expected = result["paired_errors"][split][f"{candidate}_vs_{base}"]
        if any(int(row[key]) != value for key, value in expected.items()):
            raise ValueError("Song F4 paired-error CSV does not replay")
    audit = {
        "status": "all_song_f4_trial_scores_and_pairs_recomputed",
    ...
    (directory / "F4_INCREMENT_VERIFICATION.json").write_text(
