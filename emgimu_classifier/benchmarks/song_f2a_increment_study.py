"""Exploratory F2a conditional increment on the user's Song 8-channel sessions.
...
from emgimu.feature_bank.families import (
    LocalDetailFamily, SpdTangentFamily, TraceCovarianceFamily,
)
...
ARMS = {
    ...
    "F0_plus_SPD": ("F0", "SPD"),
    "F0_plus_F2a": ("F0", "F2a"),
    "F0_plus_SPD_plus_F2a": ("F0", "SPD", "F2a"),
}
...
def run(source: Path, baseline_path: Path, output: Path) -> dict:
    ...
    families = {"F0": LocalDetailFamily().fit(source_batch),
                ...
                "F2a": TraceCovarianceFamily().fit(source_batch)}
    ...
    prediction_path = output / "F2a_INCREMENT_TRIAL_PREDICTIONS.csv"
    rows = validation_rows + final_rows
    with prediction_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]), lineterminator="\n")
        writer.writeheader()
        writer.writerows(rows)
    increment_path = output / "F2a_INCREMENT_CONDITIONAL.csv"
    with increment_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerows({"split": split, "base_arm": base,
                         ...
                         for base, candidate in (("F0", "F0_plus_F2a"),
                                                 ("F0_plus_SPD", "F0_plus_SPD_plus_F2a")))
    paired_path = output / "F2a_INCREMENT_PAIRED_ERRORS.csv"
    ...
    result = {
        "status": "exploratory_song_one_person_one_day_f2a_conditional_increment",
        ...
        "boundary": "Only Song one-person/one-day cue-labelled stable formal trials. S01/S02 source-only family/scaler/model fits, S03 validation, S04 final loaded after fixed-model S03 checks; S04 was previously inspected. S01-S03 failed whole-session collection readiness. No live USB, new-wearing, multiuser/day or deployable accuracy claim. F2a is the current trace-normalized covariance candidate, not historical Spatial Coordination equivalence.",
    }
    (output / "F2a_INCREMENT_RESULTS.json").write_text(
        json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"S03_F0_SPD_F1": validation["F0_plus_SPD"]["macro_f1"],
                      "S03_F0_SPD_F2a_F1": validation["F0_plus_SPD_plus_F2a"]["macro_f1"],
                      "S04_F0_SPD_F1": final["F0_plus_SPD"]["macro_f1"],
                      "S04_F0_SPD_F2a_F1": final["F0_plus_SPD_plus_F2a"]["macro_f1"]}),
          flush=True)
    return result
