...
def summarize(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "late_rest_intervals_with_active_decoded_state": sum(row["pre_prompt_activation"] is True for row in rows),
...
def run(source: Path, bundle: Path, collection_root: Path,
    ...
    with (ROOT / f"{prefix}_ROWS.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0]))
        writer.writeheader()
        writer.writerows(rows)
    (ROOT / f"{prefix}_AUDIT.json").write_text(json.dumps(artifact, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({key: artifact[key] for key in (
        "eligible_events", "active_events", "active_ever_correct", "active_endpoint_correct",
        "late_rest_intervals_with_active_decoded_state", "successful_active_first_correct_median_seconds")}), flush=True)
    return artifact
...
if __name__ == "__main__":
    ...
    run(args.source, args.bundle, args.collection_root, args.protocol, args.prefix)
