...
def plot(path: Path, values: np.ndarray, rate: float) -> None:
    ...
    for i,(title,series) in enumerate((("Raw EMG channel 1",raw),("50 ms RMS envelope",env),(f"Periodogram 0-{rate/2:g} Hz",np.log10(psd+1e-300)))):
        top=50+i*230; panels += [f'<text x="70" y="{top-10}" font-size="17" font-family="Arial">{title}</text>',f'<rect x="70" y="{top}" width="900" height="175" fill="white" stroke="#999"/>',polyline(series,top)]
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for user in (3,10,18):
            for speed in ("slow","fast"):
                ...
                first=values if first is None else first; samples.append(qc(values,member,200.0))
    svg=output/"semg_manus_raw_envelope_psd.svg";plot(svg,first,200.0)
    (output/"semg_manus_sanity.json").write_text(json.dumps({"dataset":"sEMG-MANUS","status":"ok","sample_strategy":"users 3, 10, 18; session 1; fist slow/fast","samples":samples,"plot":str(svg)},indent=2),encoding="utf-8")


def shift(archive: Path, output: Path) -> None:
    samples=[];first=None
    with zipfile.ZipFile(archive) as z:
        for subject in (0,10,20):
    ...
    (output/"electrode_shift_sanity.json").write_text(json.dumps({"dataset":"LibEMG Electrode Shift","status":"ok","sample_strategy":"subjects 0, 10, 20; before/after trial_1; close rep 0","samples":samples,"plot":str(svg)},indent=2),encoding="utf-8")
